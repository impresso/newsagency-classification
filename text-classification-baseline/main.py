...
def train(args, train_dataset, model, tokenizer, labels):
    ...
    t_total = math.ceil(len(train_dataset) / args.train_batch_size) * args.epochs  # assume 10 epochs
    num_warmup_steps = math.ceil(t_total * 0.1)  # 10% of training steps for warmup
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss = sequence_result.loss  # model outputs are always tuple in pytorch-transformers (see doc)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        results, _ = evaluate(args, model, train_dataset, labels, mode="dev")
...
def run(model,
    ...
    for data in tqdm(data_loader):
        ...
        _, sequence_preds = torch.max(sequence_logits, dim=1)
...
if __name__ == '__main__':
    ...
    model = ModelForSequenceAndTokenClassification.from_pretrained(args.model_name_or_path,
                                                                   config = config,
